...
#define _NMLOG(level, ...) \
	G_STMT_START { \
		if (gl.log_level >= (level)) { \
			g_print ("nm-airvpn[%s]: %-7s [helper-%ld] " _NM_UTILS_MACRO_FIRST (__VA_ARGS__) "\n", \
			         gl.log_prefix_token ?: "???", \
...
helper_failed (GDBusProxy *proxy, const char *reason)
{
	...
	_LOGW ("nm-airvpn-service-openvpn-helper did not receive a valid %s from openvpn", reason);
...
main (int argc, char *argv[])
{
	...
	gchar *bus_name = NM_DBUS_SERVICE_AIRVPN;
	gsize size;
	...
	/* Send the config info to nm-airvpn-service */
